...
# ==============================================================================
# Tray Icon & GUI Initialization
# ==============================================================================

def create_tray_icon_image():
    """Generates a simple vector-style icon for the system tray."""
    image = Image.new('RGB', (64, 64), color=(32, 33, 36))
    draw = ImageDraw.Draw(image)
    # Blue circle
    draw.ellipse([8, 8, 56, 56], fill=(138, 180, 248))
    # Dark play button triangle
...
def setup_tray_icon():
    ...
    menu = pystray.Menu(
        pystray.MenuItem("Show Log Window", show_log_window, default=True),
        pystray.MenuItem("Open Download Folder", open_download_folder),
        pystray.MenuItem("Change Download Folder", change_download_folder),
    ...
    # CRITICAL FIX: Run pystray in a daemon thread to prevent blocking tkinter
    threading.Thread(target=tray_icon.run, daemon=True, name="PystrayThread").start()
    terminal_log("[+] System tray icon initialized successfully.")

def exit_app():
    """Gracefully shuts down the tray icon, tkinter, and the application."""
    terminal_log("[*] Initiating graceful shutdown...")
    
    global tray_icon, tk_root
    
    # 1. Stop the tray icon
    if tray_icon:
        try:
            tray_icon.stop()
        except Exception:
    ...
    # 3. Exit the Python process cleanly
    sys.exit(0)

def main():
    global tk_root
    tk_root = tk.Tk()
    tk_root.title("YT-DLP Manager - Log Console")
    tk_root.geometry("700x450")
    tk_root.protocol("WM_DELETE_WINDOW", hide_log_window)
    text_widget = scrolledtext.ScrolledText(
        tk_root, 
        state='disabled', 
    ...
    terminal_log("[*] Application started. Waiting in system tray.")
    tk_root.mainloop()
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == '--host':
        ...
        setup_console_tray_behavior() # Hides the console window
        ensure_binaries_exist()
        native_messaging_loop() # Blocks here until Chrome disconnects
    else:
        main()